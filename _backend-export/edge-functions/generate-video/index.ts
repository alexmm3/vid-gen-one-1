...
interface GenerateVideoRequest {
    ...
    user_prompt?: string;
    /** Client-detected aspect ratio of the uploaded image (e.g. "9:16", "16:9", "1:1") */
...
interface EffectRecord {
    ...
    name: string;
    is_active: boolean;
    requires_secondary_photo: boolean;
...
serve(async (req) => {
    ...
    try {
        ...
        logger.info('request.parsed', {
            metadata: { device_id, effect_id, has_prompt: !!user_prompt, has_secondary: !!secondary_image_url, detected_aspect_ratio }
        });

        if (!device_id || !effect_id || !input_image_url) {
            logger.warn('validation.failed.missing_fields', { metadata: { device_id: !!device_id, effect_id: !!effect_id, input_image_url: !!input_image_url } });
            return new Response(
        ...
        if (genError) {
            ...
            return new Response(
                JSON.stringify({ error: "Failed to create generation record", request_id: logger.getRequestId() }),
                { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
            );
        }

        logger.setGenerationId(generation.id);
        logger.info('generation.created', { generation_id: generation.id });
        ...
        const grokDuration = (effectParams.duration as number) ?? providerCfg.default_duration ?? 10;
        const grokResolution = (effectParams.resolution as string) ?? providerCfg.default_resolution ?? "720p";
        const grokAspectRatio = resolveTargetAspectRatio({
            effectForcedAspectRatio: effectParams.forced_aspect_ratio as string | undefined,
            detectedAspectRatio: detected_aspect_ratio,
            effectDefaultAspectRatio: effectParams.aspect_ratio as string | undefined,
            providerDefaultAspectRatio: providerCfg.default_aspect_ratio,
        });
